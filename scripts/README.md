...
| `classify_sources.py` | Marks each archived source public or private and writes each state folder's `.gitignore`, so the text of news articles never enters git; public records are published. `--check` fails if any private copy is tracked. Run it after saving sources and before committing. |
